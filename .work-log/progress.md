## 2026-08-23
- Pi SSH 로그인 미스터리 추적: 라즈베리파이가 Opal LAN에 유선 연결되어 있다는데 station dump/DHCP/ARP 어디에도 안 잡힘. SD카드를 노트북에 꽂아 boot 파티션의 cloud-init 설정을 직접 확인 → **계정명이 `pi`가 아니라 `capstone`**임을 확인 (지난 세션 추측이 맞았음)
- **중요 재해석**: 어젯밤 극한 부하 테스트에서 "새 공기계"로 썼던 `192.168.8.109`(hostname "CapsTone")가 새 폰이 아니라 **라즈베리파이 자체**였음을 MAC 벤더 조회(`d8:3a:dd:48:55:97` → Raspberry Pi Trading Ltd)로 확인 — 즉 그땐 파이가 이더넷으로 정상 동작 중이었음
- 오늘은 케이블/SD카드 여러 번 재연결해도 Opal에도 집 공유기(SK_0600_5G) Wi-Fi에도 안 잡힘. 초록 활동 LED가 "깜빡이다 꺼짐" 관찰 → SD카드 접촉 불량으로 인한 boot 실패 패턴 추정. boot 파티션(FAT32)은 노트북에서 읽히지만 루트 파티션(ext4)은 확인 불가라 완전한 결론은 못 내림 — TV 연결로 화면 직접 확인이 필요하나 오늘은 시간상 보류
- 그 사이 데이터 수집 계속 진행: 폰 2대(`192.168.8.103`, `192.168.8.191`)로 진짜 독립적인 2-station 동시 부하 재현(어제 발견한 "노트북이 발신 허브라 가짜 경합이었다" 문제 해결) → 834행 → **1254행**으로 확대
- 라벨 재계산 결과 **label 3(심각) 4개 → 13개**로 대폭 증가, val/test에도 각 2개씩 포함되어 처음으로 통계적으로 유의미한 평가 가능. 재학습 후 confusion matrix 확인 → 모델이 이제 4개 클래스 전부 예측(전엔 2/3 완전 collapse). label 3 정확도 0%→50%, val balanced accuracy 54.4%→69.4%
- 부작용 발견: label 3 클래스 가중치가 너무 세서(train 9개뿐) label 2를 3으로 오버슈팅하는 경향 생김(label 2 정확도 19%로 하락) — 다음에 가중치 완화 필요
- 폰 iperf3 서버가 화면 꺼짐으로 두 번 죽음(`termux-wake-lock` 안 걸어둔 폰) → 재시작 대기 중 세션 일시 중단, 여기서 기록 저장
