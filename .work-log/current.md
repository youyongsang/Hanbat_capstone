...
**남은 것**: s1b(+세션 1 idle) 라벨 재계산·게이트 적용·bitrate feature 세션 간 불안정 원인 확인 / 탐색용 평가(기존 모델을 깨끗한 새 세션에 적용) / TEST 전용 세션 s2(다른 시간대·AP 휴식 후, heavy75 2회+소패킷 반복으로 심각 독립 구간 확보) / 집 2.4GHz 세션 / 학교 방문 / Pi 재부팅 원인. 데이터·CSV·러너는 커밋하지 않음(컨벤션).
