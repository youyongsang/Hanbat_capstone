...
**남은 것**: 5GHz 채택 여부 미정(발표·보고서 미반영 유지) / Pi 지연 재측정(5GHz) / 학교 수집 / 5GHz에서 어떤 보조 feature가 왜 약한지 미분해 / test가 `heavy75`에 쏠린 효과 분리(heavy75 제외 학습·평가) 미실시.
**산출물**: `project/.tmp/exp_5g/`(`run_5seed_training_6993.sh`, `metrics_5g_6993.py`, `analyze_l3_causes.py`, `occ_only_ablation_5g.py`, `eval_6993.txt`, `occ_only_5g.log`), 데이터·체크포인트는 커밋 안 함.
