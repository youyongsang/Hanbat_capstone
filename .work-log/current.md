...
**남은 것**: (라벨 재계산·bitrate 원인 확인은 완료) 세션 1c 데이터로 탐색용 평가(기존 모델을 새 세션에 적용) / TEST 전용 세션 s2(다른 시간대·AP 휴식 후, heavy75 2회+소패킷 반복으로 심각 독립 구간 확보) / 집 2.4GHz 세션 / 학교 방문 / Pi 재부팅 원인. 데이터·CSV·러너는 커밋하지 않음(컨벤션).
