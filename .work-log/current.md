...
**다음 세션 할 일**:
1. `project/.tmp/exp_5g/` 내 relabeled CSV 전부(heavy80은 cleaned 버전 사용) 병합 → 5GHz 전용 데이터셋 구성.
2. `prepare_ap_metrics_dataset.py`로 windowed 변환 → 3모델(Baseline/SDN/EE) 5시드 학습.
3. 2.4GHz 결과와 비교(정확도·L3 recall·Pi 지연 재측정은 5GHz 라디오 기준으로 별도 필요).
4. 60M/200B(load avg 3.40) 안전성 재검증 여부 결정 — 소패킷 강한 반응 카드를 더 쓸지, 여기서 멈출지.
