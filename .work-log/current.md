...
## ⭐ 다음 세션 시작 지점 — 학교 랩실 AP 실측 + telemetry feature 추가 시험

**사용자 지시(2026-09-10)**: 22차 클린 실험으로 "데이터 더 모으기" 카드는 기각됨(leakage 제거하니 이득 ≈ 0). 다음 세션은 **내일(9/11) 학교 랩실에서 이 Opal AP를 학교 공유기에 랜선 연결하고 데모+수집** — Opal 내부 토폴로지(Opal↔Pi↔노트북/폰)는 전부 동일, 학교 공유기는 그냥 상위 WAN. 목표: ① 데모 재현 확인, ② `collect_metrics.py`에 station dump의 **`expected throughput` + MCS index** feature 추가 후 회색지대(occ 58~74) 수집 → 7→9 feature 재학습으로 혼잡↔심각 변별력 오르는지 시험(유일하게 안 해본 카드). per-station airtime·`channel receive/transmit time`은 이 Opal(MT7621A, iw 4.14)에서 안 나옴 — 확인됨.
- 22차 산출물: `project/.tmp/exp_home/`(build_clean.py·run_clean.sh·agg_clean.py·data_clean_{ctrl,treat}·ck_clean_*). 커밋 안 함.
- 22차 클린 실험 결론 표는 아래 §22차. **1차 leaky 95.3%는 인용 금지** — leakage 아티팩트.
- 이전 목표(발표 보고서 작성)는 18~21차로 자료 준비 완료 상태, 보고서 착수는 정확도 시도가 끝난 뒤로 미룸.

**보고서 참고 자료(그대로 유효)**: `docs/캡스톤디자인I_최종발표.pptx`, CLAUDE.md "정량적 목표", 18~20차(k2m2 재배포 서사), `docs/yongsang/model_results.html`·`sdn_comparison.html`, §21차 다이어그램 프롬프트.
...
- **±1 허용 99.2% / 정상↔심각 혼동 0건 / 오답 전부 인접 등급** — "거친 혼잡 레벨 → 채널 전환 판단"엔 충분하다는 재프레이밍은 여전히 유효.
