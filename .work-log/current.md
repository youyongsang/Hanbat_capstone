...
**18~19차 + 데모 실기기 검증(API 레벨) + packet_size 축 추가까지 완료**: k2m2 재배포 전 계층 완료, 데모 웹사이트 참가형 재작성 완료 + **API 레벨 실기기 검증 통과**(curl로 `/check`·`/load`·`/events` 전부 확인) + **패킷 크기(1400B/200B) 축 추가로 "bitrate만으론 다축처럼 안 보인다"는 피드백 해소** (10M/1400B "정상" vs 10M/200B "혼잡" 실측 대비). `load` 필드 wire format이 문자열(`"10M"`) → 객체(`{"rate":"10M","packet_size":200}`)로 바뀌었음 — 아래 예시는 옛 포맷이니 §19차 후반부 "packet_size 축 추가" 참고. **남은 건 브라우저 UI를 사람이 직접 열어서 보는 것뿐** — Claude-in-Chrome 확장 미연결로 이번 세션엔 시각 확인을 못 함.
