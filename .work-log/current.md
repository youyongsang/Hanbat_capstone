...
### 28차 후속 — 2.4GHz 독립 세션 1 (`0920_g1`, 09-20 19:42~21:21) 수집 + 사고·판정 오류 정리

**결과**: 유효 데이터 **1,044행, 심각(게이트 전) 338행**, 원본 `project/.tmp/exp_24g/session_0920_g1/metrics_home24g_0920_g1_*.csv`(Pi `~/ap_pi_v2/`에도 보관). 미실행: `spstep_b`(심각 구간이 충분해 생략).

| 런 | 행 | 라벨 0/1/2/3 | bitrate 중앙값 |
|---|---:|---|---:|
| idle | 156 | 156/0/0/0 | - |
| step_a (10→40M×60s, 1200B) | 225 | 36/107/30/52 | 58.5 |
| spstep_a (10→40M, 300B) | 148 | 15/53/36/44 | 60.0 |
| spknee_a (22M×2, 200B) | 164 | 29/6/30/99 | 1.0(신뢰 불가, 아래) |
| spknee_b | 162 | 37/0/31/94 | 56.0 |
| step_b | 189 | 34/19/87/49 | 54.8 |

소패킷 knee는 두 번 모두 심각 58~60%로 재현(프로브 실패·ping 160~190ms), step은 23~26%, spstep 30%. 주변 채널 이용률(idle) 16~23%. 보존한 부분 파일(풀 제외): `step_a_partial_falseabort`(~200행), `spstep_a_partial_apcrash`(56행), `spknee_a_partial_laptop_left24g`(~2분분). 풀 구성 시 마지막 줄이 깨진(열 1개) 행은 라벨 NaN으로 걸러야 함.

**사고와 판정 정정 (중요)**:
1. **가짜 크래시 판정(19:52)**: 2.4GHz 부하 중 노트북 Wi-Fi 링크가 포화돼 노트북발 ping이 298ms/손실 → 러너가 AP 크래시로 오판·중단. 5GHz에서는 링크 여유로 드러나지 않던 문제. 조치: 2.4GHz는 무응답 연속 4회(FAILMAX=4)·ping 3회 중 1회라도 응답하면 생존 처리.
2. **"AP 크래시"(20:03)의 진위는 불확실**: 그때 노트북이 집 공유기(`SK_0600_5G`)로 옮겨 가 있어 노트북에서는 AP·Pi·폰 전부 접근 불가가 당연했고, **독립 경로(유선 Pi에서 AP ping)로 확인하지 못함**. 재부팅으로 복구된 것만으로 크래시를 입증하지 못함. **5GHz 세션 1c의 "AP 크래시"(02:53, 역시 노트북이 `SK_0600_5G`로 이동)도 같은 모호함** → 노트북 한 곳에서만 본 "AP 크래시" 결론은 재검토 대상.
3. **노트북이 부하 중 2.4GHz를 벗어나 5GHz SSID로 자동 이동(20:30)**: 노트북에 `GL-SFT1200-a08`·`GL-SFT1200-a08-5G` 등 프로필이 모두 저장돼 있어 2.4GHz 링크 포화 시 Windows가 자동 로밍 → `spknee_a` 1차 시도 오염(부분 파일로 격리, 재수집). 조치: 러너에 `EXPECT_SSID` 확인(시작 전 + 부하 중 다른 SSID로 이동 시 즉시 중단, 빈 조회는 무시) 추가. **자동 연결 끄기는 아직 미적용(사용자 결정 대기)**.
4. **노트북 tx bitrate 고착 재발 (AP측 rate 보고)**: AP 재부팅 직후 새 association에서 6.5Mbps×3 → 러너 사전 확인이 중단 → Wi-Fi 재연결로 해소. 이후 `spknee_a` 뒤에는 **재연결 2회 후에도 idle에서 1.0/15/21.7Mbps로 불안정**(노트북 쪽은 신호 90%·rx 270Mbps로 정상, AP `expected throughput` 67.6Mbps → 링크는 살아 있고 AP측 rate 보고만 고착) → **Wi-Fi 어댑터 끄기/켜기로만 해소**(90Mbps 정상, 연결 시간 새로 시작). 절차: 단순 재연결 → 안 풀리면 어댑터 초기화. `spknee_a`의 bitrate 값(0/1/20/39Mbps)은 시작 전 고착이 남았을 가능성이 있어 그 런의 `sta_tx_bitrate_mean`은 신뢰 불가(라벨은 loss/latency/occupancy 기반이라 영향 없음). 앞서 내가 붕괴 구간의 rate 0~1Mbps를 "링크 실제 붕괴"로 해석한 것은 근거 부족.
5. **러너 버그/오탐 (수정 완료, `project/.tmp/exp_5g/run_profile.sh`)**: (a) bitrate 검사 "중앙값<20 = 고착"이 포화 프로파일의 정상 데이터를 고착으로 오판(spknee_a 값 0/1/20/39) → **6.5Mbps 부근(6.0~7.5)에 부하 행의 60%↑가 몰릴 때만 고착**으로 변경. (b) 런 종료 시 잘린 마지막 줄(열 1개)에서 요약 스크립트가 `None`으로 죽고 빈 값을 고착으로 오판 → 깨진 행 무시·빈 값은 NA. (c) `export SESS=… OUT=…session_${SESS}`를 같은 줄에서 써서 `OUT`에 세션 ID가 빠짐(`session_/`) → 줄 분리, 파일은 `session_0920_g1`로 통합. (d) SSID 조회의 `tr -d ''`이 파일에 진짜 줄바꿈으로 들어가 코드가 끊겨 있었음 → `get_ssid` 함수로 교체, netsh 빈 응답 재시도. (e) **패치 스크립트가 `AssertionError`로 실패했는데 뒤 명령이 이어 실행돼 수정 전 러너로 세션이 시작됨** → 교훈: 패치 성공을 확인한 뒤 드라이버 실행.
6. **Pi 다운(21:21, 부하 종료 직후)**: 부하 종료 후 수집기 마무리 시점에 Pi ping 0%·SSH 타임아웃(AP측 DHCP 임대·ARP·이더넷 포트 링크는 정상) → `step_b` 복사 실패(`rc=7`)로 세션 중단. 사용자 재부팅(22:11) 후 SD카드의 파일 전부 무사, 복사 완료. **5GHz 세션 2에서도 Pi 재부팅 1회 → Pi 다운 2회째, 원인 불명**(전원·SD카드·과열 등 미확인).

**러너/환경 변경(2.4GHz용)**: `run_profile.sh`에 `BAND`(기본 5g)·`IFACE`(wlan1)·`PHONES`(s21 s26)·`FAILMAX`(2)·`LOADTHR`(100, 2.4GHz는 30)·`EXPECT_SSID` 환경변수 추가, 드라이버 `project/.tmp/exp_24g/run_session_24g*.sh`(`BAND=24g IFACE=wlan0 PHONES="s21g s26g" FAILMAX=4 LOADTHR=30 EXPECT_SSID=GL-SFT1200-a08`). 폰 SSH 별칭 `s21g`(192.168.8.191)·`s26g`(192.168.8.103)는 `~/.ssh/config`에 추가.

**수집 체크리스트 추가/갱신**: 노트북 전원 연결·절약 모드 끔·폰 손대지 않기(기존) + **노트북 Wi-Fi가 목표 SSID에 있는지 확인, 다른 저장 프로필 자동 연결 끄기 권장** + **bitrate 고착 시 재연결 → 어댑터 초기화** + **크래시 주장은 유선 Pi에서 AP ping 등 독립 경로로 확인** + 재시작 전 Pi 수집기 프로세스 0개·패치 성공 확인.

**2.4GHz 세션 1(`g1`) 독립 test 평가 — 기존 배포 모델(k2m2 5시드)을 세션 단위로 떨어진 새 세션에 적용** (재학습 없음, 2.4GHz 캐노니컬 스케일러, 스크립트 `project/.tmp/exp_24g/eval_g1.py`, 결과 `eval_g1.txt`). 풀: 6개 런 병합 후 깨진 줄 제거 → 1,044행 → 게이트(k=2/m=2) 재라벨(`pool_g1/pool_relabeled.csv`, 심각 338→315행, 게이트로 23행 강등) → 윈도우 **972창**(라벨 0/1/2/3 = 238/185/234/**315**), L3 독립 구간 **6개**(step 3, spstep 1, spknee 2). 5시드 평균±SD %:

| 모델 | 정확도 | L3 recall | L3 precision | L3 F1 | L3 recall 95% CI(구간 부트스트랩) |
|---|---:|---:|---:|---:|---|
| Baseline | 79.7±1.9 | 60.5±5.0 | 84.1±4.8 | 70.3±4.3 | [47, 74] |
| SDN | 75.1±2.2 | 55.0±12.9 | 78.9±4.6 | 63.6±7.6 | [33, 75] |
| EE Fixed | 77.9±2.9 | 71.6±14.7 | 75.4±3.3 | 72.4±7.5 | [57, 84] |
| EE Dynamic | 75.5±3.1 | 64.3±15.0 | 72.9±2.7 | 67.2±8.8 | [45, 81] |

- **in-distribution(기존 k2m2 수치) 대비**: 정확도 91.7~92.7% → **75~80%(−12~17pt)**, L3 recall 83.8~87.6% → **55~72%(−15~30pt)**, L3 F1 84.3~86.7% → 64~72%. **세션 단위로 떨어뜨리면 2.4GHz도 뚜렷하게 하락**(5GHz 독립 test의 recall 42~48%보다는 덜 하락). 발표에서 인용하는 2.4GHz L3 recall 84~88%는 세션 내 분할 수치이고 세션 단위로는 대략 55~72%(구간 6개라 CI 매우 넓음).
- **프로파일별 L3 recall(창)**: 소패킷 step(spstep, 43창) 84~91% / spknee(182창) 46~70% / step(90창) 58~66%. **런별(4모델 평균)**: spstep_a 87.4 / spknee_b 76.3 / step_a 64.5 / step_b 58.3 / **spknee_a 39.1**.
- **심각 유형별 L3 recall(4모델 평균, 창)**: occupancy 주도(occ_score≥0.75, 244창) **73.6%** / occ 미달+프로브 실패(45창) **21.2%** / occ 미달+프로브 정상(측정된 loss·latency, 26창) **33.8%** — 텔레메트리로 안 보이는 유형이 훨씬 낮다는 예측과 일치. 2.4GHz는 심각의 78%가 occupancy 주도라 5GHz(occ 미달 49~68%)보다 덜 치명적.
- **오탐**: 예측=3인데 실제가 아닌 것의 **96%가 실제 혼잡(2)**(혼잡→심각 오탐; 실제 0/1/2 = 1/43/1,142건). 앞서 세운 "고착 런 학습 시 독립 세션에서 혼잡→심각 오탐 증가" 예측과 일치하나 이 데이터만으로는 원인을 bitrate로 단정할 수 없음.
- **`spknee_a` 제외**(그 런의 bitrate 값은 신뢰 불가): recall Baseline 68.1 / SDN 68.6 / EE Fixed 78.6 / EE Dynamic 74.7, precision 81.1/77.2/72.7/70.7 — 이 런이 가장 낮은 recall(39.1)이지만 bitrate 아티팩트 때문인지는 미확인.
- **한계**: 독립 구간 6개(step 3, spstep 1, spknee 2)로 모델 간 차이는 구분 불가, 세션이 하나(같은 날 저녁)라 날짜 효과는 못 봄. 다음: 다른 날 세션 추가(목표 독립 구간 20~30), 7-feature vs 6-feature(bitrate 제외) 모델을 이 독립 세션에서 비교(6-feature 모델은 체크포인트 저장 재학습 필요).

**2.4GHz 독립 세션 2(`g2`, 09-21 00:26~01:48) 수집 + g1+g2 평가**

*수집*: 원본 `project/.tmp/exp_24g/session_0921_g2/`. 유효 1,262행(idle 156 / step_a 216 / spstep_a 185 / spknee_a 160 / spknee_b 163 / step_b 194 / spstep_b 188), 게이트 전 심각 = step_a 57(26%) / spstep_a 78(42%) / spknee_a 113(71%) / spknee_b 92(56%) / step_b 62(32%) / spstep_b 78(41%). **프로파일별 심각 비율이 g1과 일관**(spknee 56~71%, spstep 30~42%, step 23~32%) — 2.4GHz는 5GHz(같은 프로파일 심각이 세션마다 0~75%로 갈림)와 달리 세션 간 변동이 작음. **g1과 g2는 같은 밤의 연속 세션(3시간 간격)이라 날짜 독립성은 없음**(진짜 다른 날 세션은 아직 없음).
*사고*: (1) **AP 크래시(00:33, step_a 중, AP 가동 4시간 14분)를 이번에는 독립 증거로 확인** — 러너가 AP 무응답 4/4로 중단한 뒤 AP 가동 시간이 4:14→3분으로 초기화, DHCP 임대 소실, 폰 2대가 떨어져 나감(노트북·Pi는 재연결/응답) → 노트북발 판정 오류가 아닌 실제 크래시(사용자도 확인). 부분 파일 `step_a_partial_apcrash`(124행) 보존·풀 제외. 재부팅 후 폰은 자동 재연결됨. (2) **AP 재부팅 직후 노트북 tx bitrate 고착(6.5×3) 세 번째 재발** → Wi-Fi 어댑터 초기화(끄기/켜기)로 해소 후 정상(135/90/90). **절차: AP 재부팅 후에는 처음부터 어댑터 초기화.** (3) **운영 실수 2건**: 드라이버 복사본에 `idle` 줄이 남은 채 시작(치환 패턴 불일치 — 실행 전 `grep '^run'`으로 확인해 발견·중단), 메인 드라이버(`run_session_24g.sh`)에 2.4GHz 판정 설정(FAILMAX=4, LOADTHR=30, EXPECT_SSID)과 OUT 경로 수정이 빠져 있었음(이어 돌리던 변형 스크립트에만 적용돼 있었음) → 수정 후 재시작(그 사이 `session_` 폴더가 생겨 삭제, g1 파일 무결성 확인). 교훈: 드라이버 시작 전 `grep '^export\|^run'`으로 설정·런 목록 확인. (4) 오래된 감시(Monitor)가 같은 로그를 계속 tail해 알림이 중복 도착 — 세션마다 감시 1개만 유지.

*평가* (스크립트 `project/.tmp/exp_24g/eval_g12.py`, 결과 `eval_g12.txt`; 풀 `pool_g12/pool_relabeled.csv` = g1 1,044 + g2 1,262 = 2,306행, 게이트 재라벨 후 심각 781행; 윈도우 2,150창 라벨 0/1/2/3 = 508/376/486/**780**, L3 독립 구간 13개(step 5, spstep 4, spknee 4)). 기존 배포 모델 k2m2 5시드, 5시드 평균±SD %:

| 모델 | 정확도 | L3 recall | L3 precision | L3 F1 | L3 recall 95% CI |
|---|---:|---:|---:|---:|---|
| Baseline | 82.1±1.1 | 77.3±2.5 | 82.5±2.6 | 79.7±1.8 | [64, 88] |
| SDN | 78.5±1.7 | 74.9±6.4 | 79.1±2.3 | 76.7±2.6 | [57, 88] |
| EE Fixed | 80.4±1.5 | 83.3±7.3 | 77.6±1.5 | 80.1±3.0 | [72, 92] |
| EE Dynamic | 79.1±1.7 | 79.7±7.5 | 76.8±1.4 | 78.0±3.3 | [66, 90] |

세션별 L3 recall / 정확도: **g1**(구간 6) Baseline 60.5/79.7, SDN 55.0/75.1, EE Fixed 71.6/77.9, EE Dynamic 64.3/75.5 — **g2**(구간 7) Baseline **88.6**/84.1, SDN **88.3**/81.4, EE Fixed **91.2**/82.5, EE Dynamic **90.2**/82.1. 프로파일별 L3 recall(4모델): 소패킷 step(193창) 88~93 / 소패킷 knee(382창) 72~84 / step(205창) 66~73. 심각 유형별(4모델 평균): occupancy 주도(649창) 87.8 / occ 미달+프로브 stale(96창) 31.5 / occ 미달+측정된 loss·latency(35창) 41.3. 오탐 3,318건 중 **98%가 실제 혼잡(2)**(precision 77~83, g1 오탐 중 혼잡 1142/1186, g2 2111/2132).
- **해석**: g1과 g2가 크게 다름 — **g1 55~72%, g2 88~91%(기존 in-distribution 84~88% 수준)**. 합산(75~83%)은 이 차이를 가려 대표값으로 쓰기 곤란. 세션 단위로 떨어뜨려도 g2에서는 in-distribution 수준이 유지되어 "독립 test에서 무너진다"는 g1만의 결론이 일반적이지 않을 수 있음.
- **g1이 낮은 이유는 미확정**: g1의 `spknee_a`(bitrate 값 신뢰 불가 런, recall 39.1)를 빼면 g1 recall이 68~79%로 올라가나(위 g1 단독 평가) **여전히 g2(88~91%)보다 낮음** → bitrate 오염만으로는 g1-g2 격차가 설명되지 않음. 런별 표(step_a 61.3 / spstep_a 91.3 / spknee_a 70.4 / spknee_b 86.3 / step_b 78.1 / spstep_b 89.0)는 g1·g2를 합친 값이라 세션 간 차이를 보여주지 못함(세션×런별 분해는 미실시). 검증 필요: g1·g2 런별 분해, 두 세션의 `sta_tx_bitrate_mean`·occupancy 분포 비교.
- **한계**: 독립 구간 13개(CI 넓음), 두 세션이 같은 밤이라 날짜 효과 미확인, g1·g2 모두 같은 장소·같은 AP·같은 폰 배치.
- **다음**: 다른 날 세션(g3~), g1·g2 세션×런별 분해, 7-feature vs 6-feature 모델 독립 세션 비교(6-feature 모델은 체크포인트 저장 재학습 필요).

**남은 것**: (2.4GHz g1·g2 수집·평가 완료) 6-feature 모델 체크포인트 재학습 후 7 vs 6-feature 독립 세션 비교 / 2.4GHz 세션 추가(다른 날 2~3회, 목표 독립 구간 20~30) / 노트북 자동 연결 설정 결정 / Pi 다운 원인 확인 / 5GHz "AP 크래시" 재검토.
...
### 28차 후속 (11) — 2.4GHz g3~g5 수집 + bitrate 오염 진단 + 세션 단위 평가(LOSO) (09-21)

**요약**: g3 평가에서 L3 precision이 유독 낮은 원인을 추적하다 **학습 데이터의 `sta_tx_bitrate_mean` 오염**과 **창 단위 분할 평가의 낙관성**을 확인했다. 오염 런을 빼는 것만으로는 좋아지지 않아(혼잡 예시가 같이 사라짐) **깨끗한 bitrate 조건에서 재수집**하기로 했고, 러너를 보완해 g4·g5를 수집했다. 평가 기준은 leave-one-session-out으로 바꿨다. **사용자 결정: bitrate는 feature로 유지한다.** 정리 문서: `docs/yongsang/g3_bitrate_loso_analysis.html`.

**g3 수집** (09-21 03:33~, `project/.tmp/exp_24g/session_0921_g3/`): 품질 통과 5런 966행(idle 156 / step 199 / spstep 194 / spknee 167 / gray 250[1차 36% 미달 → AP 재부팅 후 재수집 4%]). 게이트 미달로 풀 후보 제외한 런은 이름으로 보존(kneelo 28% / kneepkt500 24% / kneehiprobe 30% / kneehi 12% 미달, kneepkt500 재시도는 AP 크래시로 134행 부분). 사고: (1) 어댑터 초기화 뒤 **노트북 iperf3 서버가 죽어 spstep이 부하 없이 종료**(전 행 0)했는데 러너가 미감지, (2) **Pi의 같은 이름 CSV가 남아 재수집 시 이어 붙음**(spstep 468행 = 274+194, kneelo 330행), (3) **kneepkt500 중 AP 크래시를 처음에 "정체"로 오판** — 회복 뒤 유선 ping 정상을 근거로 삼았으나 AP uptime이 5분으로 초기화돼 있었음(크래시 판정은 uptime 초기화·DHCP 임대 소실로), (4) AP 재부팅 뒤 bitrate 고착 재발 → 어댑터 끄기/켜기(관리자 권한 필요). kneelo 1차의 100초 무응답은 uptime이 이어져 재부팅이 아니었음.

**기존 배포 모델을 g3에 적용** (`eval_g3.py`, 풀 906창·심각 195): Baseline 정확도 77.4 / L3 recall 84.2 / precision 56.0, SDN 74.4/86.9/51.8, EE Fixed 69.2/88.8/45.4, EE Dynamic 68.6/88.2/44.8. **recall은 g2 수준인데 precision이 낮음**(오탐의 94%가 혼잡→심각). gray 제외 시 precision 57~66%(gray가 일부만 설명).

**오탐 진단**: 혼잡(2) 창의 심각 오예측이 점유율 65~75% 경계에 집중(g1 7·39% / g2 42·87% / g3 61·80%). g3의 혼잡 창 bitrate 중앙값이 107.5Mbps로 g1(47.6)·g2(57.8)의 2배. **반사실**(Baseline 5시드, bitrate 입력만 g1·g2 중앙값으로 고정, 재학습 없음): g3 혼잡 오예측 43.6→12.7%, precision 65.9→86.1, recall 86.3→80.4; g1 recall 60.5→75.4(같은 원인이 g1에선 recall, g3에선 precision을 깎음); g3 bitrate를 g1·g2 95분위로 상한만 씌우면 변화 없음(분포 전체 이동). g2도 같은 처치에 개선.

**학습 데이터 오염 정량화** (캐노니컬 2,551행): step_run4·5·6이 부하 행의 62~75%가 6.0~7.5Mbps 고착(run1~3 4~8%, smallpkt 0%). **고착 런이 1,344행 = 53%**(앞서 기록한 34%는 다른 분모). **라벨 편중**: 혼잡(2) 885행 중 617(70%)이 고착 런, 심각(3) 284행 중 220(77%)이 정상 런. 그룹별 bitrate 중앙값 — 고착 런 혼잡·심각 모두 6.5 / 정상 런 혼잡 40.5·심각 57.8. 점유율 60~75% 경계에서 정상 런 안의 혼잡 40.0 vs 심각 39.7로 **변별력 없음**(7번째 feature의 근거였던 Cohen's d 0.52가 고착/정상 런 구성 차이에서 왔을 가능성, 가설). g1도 3/6 런이 미달(spknee_a 57%·중앙값 1.0 / spknee_b 34% / step_b 43%), g2·g3는 깨끗(0~10%).

**재학습 실험** (Baseline 5시드, 독립 g1+g2+g3 gray 제외 2,818창, `retrain_variants.py`): A 7feat 전체 정확도 82.0 / recall 81.8 / precision 77.4, **B 고착 런 제외 78.9 / 91.4 / 70.7(나빠짐 — 혼잡 예시 885→268행)**, C 6feat 전체 83.3 / 89.4 / 76.5, D 6feat+제외 75.9 / 93.4 / 69.3. C는 A와 동급이며 편차가 작음(g1 recall A 68±15.7 vs C 87±4.7); in-dist에선 A가 낫다. **단순 제외는 해법이 아니다.**

**평가 방식 변경**: 창 단위 분할은 이웃 창(행 11개 공유)이 train에 들어가 낙관적(in-dist 92~93% vs 독립 78~84%). 행을 섞은 뒤 창을 만드는 것은 시계열이 깨져 불가. 같은 세션의 다른 런도 링크 상태를 공유하므로 **leave-one-session-out**(세션 통째로 제외, 검증은 학습 세션마다 마지막 런을 통째로 분리)로 확정. LOSO는 성능 추정용이고 배포 모델은 고른 설계로 전 세션을 합쳐 다시 학습한 EE 하나. `loso.py`.

**LOSO 5세션 결과** (7-feature Baseline 5시드, 8/28·9/1·g1·g2·g3): 오염 포함 세션 평균 정확도 88.4 / recall 77.3 / precision 83.3, **품질 통과 런만**(8/28 step_run4~6, g1 spknee_a·spknee_b·step_b 1,787창 제외) 88.4 / 78.3 / 84.1 — 평균은 거의 안 변함. g3 precision 79.3→69.3(혼잡 예시 손실), 8/28 recall 43.6→42.5(고착 런을 뺀 깨끗한 테스트에서도 그대로 → **"8/28 recall이 낮은 건 테스트의 고착 런 탓"이라던 가설 기각**). **8/28의 낮은 recall 원인은 심각 유형 구성**: 8/28 심각은 occupancy 주도 43% / occ 미달+측정된 loss·latency 46% / occ 미달+프로브 stale 11%(이 비율은 오염 런까지 포함한 8/28 전체 런 집계 — **테스트에 쓴 품질 통과 런 105창 기준으로는 40 / 51 / 9로 정정**, 아래 EE·SDN 절 참고), 나머지 세션은 occupancy 주도 77~87%.

**g4·g5 수집** (`session_0921_g4`·`g5`, 러너 보완 후): 라벨과 세션이 엮이는 구조를 끊으려고 **같은 프로파일(idle·step·spstep·gray)을 노트북 링크 상태를 바꿔 수집**. g4(링크 좋음, 노트북 −47~−53dBm·AP측 108~135Mbps) 808행: idle 156 / step 216(37/75/44/60) / spstep 171(26/49/17/79) / **gray 265(38/41/169/17 — 깨끗한 bitrate의 혼잡 예시)**. g5(링크 약함, −61dBm·45~60Mbps) 685행: idle 156 / step 193(40/62/13/78) / spstep 153(33/0/15/105) / gray 183(34/0/11/138). 전부 bitrate 게이트 통과, AP 크래시 없음(가동 4시간대를 넘김). **g5는 링크가 약해 혼잡·경고 단계가 사라지고 바로 심각(지연 중앙값 340~430ms)으로 감 — 심각의 약 87%가 occupancy 미달 지연·손실 주도**(기존 모델이 못 잡던 유형이 대량 확보). 오염: g4 idle·step 1차는 **S26에서 Netflix 시청**(idle에 20초 간격 4~29Mbps 급증·혼잡 라벨 8행) → `_NETFLIX_contaminated`로 보존하고 재수집(재수집 idle은 156행 전부 0으로 확인).

**기존 배포 모델을 g4·g5에 적용** (`eval_pool_g45.txt`, 1,397창): **g4 정확도 62~74% / recall 81~88% / precision 43%**(혼잡을 심각으로 오판, 오탐의 87%), **g5 70~74% / recall 48~58% / precision 97%**(지연 주도 심각을 놓침). 서로 반대 방향 실패. 유형별 L3 recall: occupancy 주도 **95%** / occ 미달+프로브 stale 54% / occ 미달+측정된 loss·latency **19%**.

**LOSO 7세션 결과** (`loso.py`, 품질 통과 런만, Baseline 5시드, `loso7_clean.log`): 8/28 정확도 87.3 / recall 42.9 / precision 80.5 · 9/1 90.1 / 88.7 / 86.0 · g1 93.5 / 90.0 / 89.5 · g2 88.3 / 92.8 / 86.8 · g3 89.0 / 82.8 / 82.7 · g4 89.8 / 71.8 / 92.9 · g5 76.8 / 63.0 / 98.5 → **세션 평균 정확도 87.8 / recall 76.0 / precision 88.1**. g4·g5를 학습에 넣자 **g3 precision 69.3→82.7**(링크 상태 다양성의 효과). 기존 캐노니컬만 학습한 모델이 g4·g5에서 보인 실패(g4 precision 43, g5 recall 48~58)는 LOSO에선 g4 precision 92.9, g5 recall 63.0으로 완화. 한계: g1~g5는 같은 집·같은 AP·같은 밤이라 하나를 빼도 나머지가 학습에 남음 — 진짜 다른 날은 8/28·9/1뿐이라 g fold 수치는 낙관적. Baseline만 확인, EE·SDN 미확인. **g5 fold 혼동행렬**(5시드 합, 창 637): 실제 심각 305창 중 192가 심각, 71이 혼잡·39가 경고로 예측(recall 63%), 혼잡 51창 중 22가 경고로 → 정확도 76.8%가 낮은 건 심각 비중 48%인 세션에서 지연 주도 심각이 혼잡·경고로 읽히기 때문(정상↔심각 혼동은 3창뿐). **8/28·g1을 학습에서 통째로 뺀 LOSO 5세션**(9/1·g2·g3·g4·g5, 3,881창, `loso5_no0828g1.log`): 같은 5개 fold 평균 정확도 86.8→85.8 / recall 79.8→78.7 / precision 89.4→89.8 — 변화가 시드 편차 안(g2 −2.4pt, g4 recall −4.1pt만 다소 눈에 띔), g5·g3 recall은 그대로(63.0→64.1, 82.8→83.3). "8/28을 빼면 g5·g3 recall이 내려갈 것"이라던 예상은 틀림 → 세션 통째 제외의 이득도 해도 작아 **깨끗한 런은 학습에 유지**(창 5,109→3,881로 줄고 진짜 다른 날이 9/1 하나만 남는 점이 불리).

**러너 보완** (`project/.tmp/exp_5g/run_profile.sh`, 백업 `.bak_0921`): (1) 시작 전 노트북 iperf3 서버 5201/5202/5203 점검·재기동(실패 시 exit 14), (2) Pi의 같은 이름 잔여 CSV를 `_prev_시각`으로 이동·남은 수집기 프로세스 정리, (3) AP uptime을 10초마다 `<런>_uptime.log`에 기록하고 감소하면 재부팅으로 판정해 부하 중단(exit 15, 실제 크래시로는 미시험), (4) 품질 게이트(exit 13) 유지, 세션 드라이버에서는 rc=13을 경고로 두고 계속 진행하는 변형 사용. 수집 중 노트북·폰에서 다른 통신(유튜브·넷플릭스) 금지.

**해석·다음**: 문제는 "bitrate를 빼느냐"가 아니라 **깨끗한 bitrate의 혼잡·경계·지연 주도 심각 예시가 부족한 것**. 재수집은 (a) 깨끗한 bitrate의 혼잡과 경계(occ 55~75%) 보강, (b) occupancy 미달 loss·latency 주도 심각(관측 한계 유형), (c) 다른 날·다른 장소 세션, (d) 라벨과 링크 상태가 엮이지 않게 같은 프로파일을 여러 링크 상태로. **EE·SDN LOSO 7세션 결과** (2026-09-22, `loso_multi.py`, 품질 통과 런만·8/28·g1 깨끗한 런은 다양성 위해 학습 유지, 5시드, `loso7_ee.log`·`loso7_sdn.log`): 세션 평균 정확도 / L3 recall / L3 precision / 평균 exit — Baseline 87.8 / 76.0 / 88.1 / 3, **EE Fixed 87.3 / 75.9 / 88.3 / 1.94, EE Dynamic 86.9 / 76.8 / 86.6 / 1.78, SDN 86.7 / 76.8 / 87.6 / 1.94** → **네 모델 사실상 동급**(차이 1pt 이내, 세션 평균의 시드 편차 정확도 0.3~0.5·recall/precision 1.4~2.2pt). exit 분포(1/2/3번 출구): EE Fixed 36/34/30%, Dynamic 38/45/16%, SDN 41/24/35%. 세션별 recall은 43~94%로 갈리고 같은 세션 안 모델 간 차이는 최대 6pt(8/28) — **성능을 좌우하는 건 모델이 아니라 세션 구성**. 효율 서사(정확도 동급 + 조기 종료) 유지, 실제 속도는 Pi INT8 실측으로.

**8/28 L3 recall(43~50%)의 원인**: 테스트 오염이 아님(깨끗한 step_run1~3에서도 42.5%). 8/28 심각(품질 통과 런 105창)의 유형 구성이 다름 — **occupancy 주도 40% / occ 미달+측정된 loss·latency 51% / occ 미달+프로브 stale 9%**(이전에 기록한 43/46/11은 오염 런까지 포함한 집계, 정정). 세션별 유형 비중(occ / 측정 / stale %): 9/1 87/2/11, g1 79/12/9, g2 87/2/11, g3 83/6/11, g4 64/3/32, g5 39/33/28. **유형 비중과 recall의 세션 간 상관**(n=7): 측정된 loss·latency 비중 대 recall Pearson r = Baseline −0.90 / EE Fixed −0.89 / EE Dynamic −0.87 / SDN −0.88(기울기 약 −0.7~−0.85pt per %p, 0%에서 ~89, 50%에서 ~47~52), occ 미달 전체(측정+stale)로는 r = −0.93~−0.95(g4가 stale 32%로 설명됨). 네 모델의 선이 겹침 → 모델이 아니라 유형 비중이 recall을 좌우. 한계: 세션 7개의 상관(인과 아님, 링크 상태·프로파일·심각 창 수와 함께 변함, 8/28·g5 두 극단점 의존, 네 모델은 같은 데이터라 독립 검증 아님). "AP 텔레메트리 정보 부족" 설명은 유력하나 미확정(하드웨어별 차이·프로브 stale은 recall 54%로 부분 관측·8/28 loss 라벨의 성격 미확인). 능동 프로브 하이브리드가 이런 세션에서 얼마나 도움이 되는지는 **이번엔 검증하지 않음(향후 과제)**. 정리 문서: `docs/yongsang/loso_four_models_and_0828.html` — 03절에 **막대(세션별 "occ 미달+측정된 loss·latency" 유형 비중 %) + 꺾은선(모델별 L3 recall %)을 세션 날짜순(8/28·9/1·g1·g2·g3·g4·g5)으로 겹친 그래프** 2개(측정 유형만 / occ 미달 전체). 세션별 원자료 `project/.tmp/exp_24g/sess_type_share.csv`·`type_share_vs_recall.csv`. 이전 문서 `g3_bitrate_loso_analysis.html`의 8/28 유형 수치도 같은 정정 값(40/51/9)으로 맞춤.

**남은 것**: 전 세션 합쳐 재학습한 배포용 EE 모델 준비 / 오염 런 처리(제외 대신 가중치 하향·재수집 대체) 검토 / 다른 날·학교 등 다른 장소 수집 / 러너의 uptime 감지 실제 크래시 시험 / Pi INT8 실측. 데이터·CSV·러너는 커밋하지 않음(컨벤션). **5GHz 실험 산출물은 커밋하지 않기로 결정(2026-09-22)**: 5GHz는 미채택 실험 라인이라 `project/checkpoints/ap_v2_5g_home`·`ap_v2_5g_home_6993rows`·`ap_v2_5g_home_no75`(각 33MB)와 `project/data/ap_metrics_v2_5g_home`(12MB)·`ap_metrics_v2_5g_home_no75`(7.1MB), 합계 약 118MB를 로컬에만 두고 `.gitignore`에 추가했다. 결과 수치와 해석은 §26~28차에 남아 있으므로 파일이 없어도 기록은 유지되며, 재현이 필요해지면 `project/.tmp/exp_5g/`의 스크립트로 다시 만들거나 이 파일들을 커밋한다(다른 노트북으로 옮기면 로컬 파일은 사라지니 주의).

### 28차 후속 (12) — 7세션 배포 재학습 + ONNX/Pi 지연 재측정 + AP DHCP 고정 임대 + 학교 수집 준비 (09-22)

**배포용 재학습**: 새 스크립트 `project/scripts/build_multisession_dataset.py` — 7세션(8/28·9/1·g1~g5, 품질 통과 런만, 오염 런 6개 제외) 풀을 **런 단위**로 분할한다(각 세션의 마지막 런 → val, 나머지 → train; 창 셔플 없음). test.csv는 val의 사본(held-out 세션이 없으므로 성능 지표 아님, ONNX/Pi 벤치 도구가 test.csv를 읽어서 존재만 유지). train 3,882창 / val 1,227창. 스케일러는 캐노니컬 그대로 재사용(Pi 실시간 추론 스케일러 불변). Baseline·EE·SDN 각 5시드 학습(hidden 128, 50에폭, 기존과 동일 설정) → **val balanced accuracy 최고 시드는 세 모델 모두 시드1**(Baseline 91.7, EE 91.6, SDN 91.9, T=0.79). 산출물 `project/checkpoints/ap_v2_redesign2_7sess_20260922/`(시드별은 gitignore, `selected/`만 커밋).

**ONNX 재수출·검증**: 기존 6개 스크립트(`export_onnx_ap*.py`) 그대로 재사용, `selected/`에 27개 파일(fp32/INT8, staged/unified) 생성. val 1,227창으로 PyTorch와 비교 — fp32 unified는 EE Fixed·Dynamic 둘 다 **1,227/1,227 완전 일치**, INT8은 3~4창(0.3%)만 차이(정확도 차 0.1~0.2pt). 스크립트 `project/.tmp/exp_24g/verify_onnx_7sess.py`.

**Pi 지연 재측정** (INT8, Pi 4코어·유휴·스로틀링 없음, 기존 배포 모델과 A/B/A/B 2라운드): **EE Fixed·Dynamic은 기존과 동속**(0.62~0.65ms, Baseline 대비 약 −25%). **SDN만 8~26% 느려짐**(0.58→0.73ms) — 선택된 시드의 보정 임계값 T=0.79가 기존 T=0.72보다 높아 exit3 비율이 30%→51~56%로 늘어난 임계값 정책 효과이지 구조 변화 아님(SDN 속도는 매번 T에 크게 좌우됨, 9차·이번 둘 다 같은 결론). Baseline은 구조 동일이라 무변화. 결과 `project/results/yongsang/ap_v2_redesign2_7sess_pi_latency_20260922.txt`.

**g3~g5에서 새 모델이 유효한지 확인** (`eval_new_by_session.py`, 기존 모델과 같은 창 비교): 학습에 쓴 런에서 g3~g5 합산 EE Fixed 정확도 70.7→91.4 / L3 recall 81.9→90.5 / precision 51.6→83.5(기존→새). **그래디언트 학습에 안 쓴 검증 런**(세션별 마지막 런, 613창·심각 206)에서도 88.3/90.3/86.9로 유지 — 기존 모델의 실패(g4 precision 29%, g5 recall 67% 등)가 사라짐. **단, 이 검증 런도 같은 세션의 다른 런이 학습에 있고 val 자체가 에폭 선택에 쓰였으므로 진짜 held-out이 아님** — 새 모델의 정직한 기대 성능은 여전히 LOSO(g3 89.0/82.8/82.7, g4 89.8/71.8/92.9, g5 76.8/63.0/98.5, §28차 후속 11)를 인용. 8/28·9/1(기존에 잘하던 세션)은 새 모델에서 2~3pt 소폭 하락 — 세션 다양성 확보의 작은 비용.

**AP DHCP 고정 임대**: 노트북(192.168.8.226)·S21(191)·S26(103)·Pi(109)를 MAC 기준 고정(`/etc/config/dhcp`, 백업 `dhcp.bak_0922`, dnsmasq reload만·재부팅 없음). 수집기(`collect_metrics.py`)·러너(`ramp_load_remote.sh`)에 이 IP들이 하드코딩돼 있어 DHCP가 다른 주소를 주면 부하·프로브가 조용히 엉뚱한 곳으로 감 — 다른 장소(학교) 수집을 앞두고 선제 조치. **주의**: 폰 MAC은 랜덤 주소라 Wi-Fi를 "잊고" 재연결하면 MAC이 바뀌어 고정이 깨질 수 있음, 학교에서 폰 Wi-Fi 설정 건드리지 말 것.

**러너 IP 점검**: `run_profile.sh`(백업 `.bak_0922`)에 preflight 단계로 노트북 Wi-Fi IP가 기대값(기본 192.168.8.226, `EXPECT_LAPTOP_IP`로 override)과 다르면 즉시 중단(exit 16) 추가 — 잘못된 IP/올바른 IP 두 경우 모두 시험 통과.

**다음**: 내일(09-23) 학교에서 g6 세션 수집 — 새 모델의 진짜 held-out 시험(학습에 전혀 안 들어간 세션 + 다른 장소). 예상: 학교 idle occupancy가 기존에 45~50%로 관측된 바 있어(23차) **정상(0) 라벨이 거의 없고 경고 바닥에서 시작**, 부하가 조금만 걸려도 혼잡·심각으로 빨리 상승 — 집 세션과 합산하지 말고 별도 세션으로 보고할 것. 현장 체크: idle 2분으로 배경 점유율 먼저 확인, 폰 2대는 AP에서 비슷한 거리 유지(크래시 방지), 가능하면 Opal 채널을 덜 붐비는 쪽으로.

**남은 것**: g6(학교) 수집·평가 후 배포 여부 최종 결정 / 새 모델로 `deploy/raspberry_pi_ap_v2/`·Pi 라이브(`live_congestion.py`, 데모) 교체는 g6 결과 확인 후 / EE·SDN도 g3~g5 세션별 표로 CLAUDE.md·work-log 수치 갱신 / 원본 수집 CSV(g1~g5) 백업(현재 이 노트북에만 있음, 유실 위험).

### 28차 후속 (13) — 학교 독립 세션 g6 수집 + 새 배포 후보 모델 첫 held-out 시험 (09-23)

**수집**: 학교(집과 다른 장소, 첫 방문), **S21을 집에 두고 와 S26 한 대로만 진행**(`PHONES` 기본값이 러너 스크립트 안에 `"s21g s26g"`로 하드코딩돼 있어 외부에서 env override가 안 먹힘 — 원격으로 스크립트를 직접 고쳐 `PHONES="s26g"`로 픽스, 이미 성공한 idle 런도 재실행 없이 스킵하도록 수정). idle 156행(정상 135·경고 21) → step 263 → spstep 245 → gray 332, 합계 996행, 전부 bitrate 품질 게이트 통과(0~1% 미달), AP 크래시 없음. **원격 지원 프로토콜**: 학교는 인터넷 없이 Opal 공유기만 운용 예정이라 이 세션이 실시간으로 붙어있지 못할 것으로 예상해 사전에 복붙용 실행 가이드(설정 체크리스트, exit 코드별 대응표, IP/iperf3 수동 복구법)를 채팅으로 전달 — 실제로는 학교에 와이파이가 있어 세션이 계속 연결됐고, PHONES 하드코딩 문제도 이 세션이 원격으로 직접 스크립트를 고쳐 해결.

**학교 idle 배경**: 정상 135/156(87%) — **23차 학교 랩실 idle 45~50%와 다르게 조용함**(다른 건물/시간대/기기 밀도 차이로 추정, 원인 미확인). 부하를 걸자 대부분 경고(1)에 머무름(step 84%, gray 78%) — 폰 1대라 부하가 절반이었던 영향이 커 보임. 혼잡(2)은 step 7·spstep 41·gray 33, **심각(3)은 4개뿐**(step 1·spstep 3, 게이트 후 표본 극소).

**평가** (`eval_g6.py`, 기존 배포 모델 vs 7세션 재학습 새 모델, 창 948개): 정상~혼잡 구간은 **새 모델이 전반적으로 개선**(정확도 Baseline 89.8→95.0, EE Fixed 92.8→93.4, SDN 90.2→92.9, EE Dynamic만 93.1→92.8로 보합; 혼잡(2) recall Baseline 77.8→93.8%·SDN 77.8→91.4%로 크게 개선). **심각(3, n=4)은 새 모델 전부 recall 0%**(기존 모델은 25%, 4개 중 1개) — 표본이 너무 적어 통계적 의미는 없음, 오답은 전부 경계(1↔2)로 가까운 거리·정상↔심각 혼동은 0건. **해석**: g6는 "학습에 전혀 안 들어간 다른 장소 세션에서 정상~혼잡 구간이 무너지지 않는가"에는 긍정적 답을 주지만(배포 교체를 지지), "다른 장소에서 심각을 잡는가"는 표본 부족으로 검증하지 못함 — 폰 1대·약한 부하 탓에 심각까지 못 밀어붙인 한계.

**다음**: 새 모델을 `deploy/raspberry_pi_ap_v2/`·Pi 라이브로 교체할지 최종 결정 — 심각 검증이 안 된 채로는 g6만으로 확정하기엔 이름. g6도 학습 풀에 합류시켜 재학습할지 검토(현재는 미포함, 순수 시험용으로 둠). 다른 장소·폰 2대 조건의 후속 세션이 있으면 심각 구간 검증 보강 가능.

**g6를 학습 풀에 넣을지 검토 (결론: 지금은 보류, g7 이후 재검토)**. bitrate 오염 여부부터 확인 — g6 부하 행 bitrate 중앙값 57.8Mbps로 g1(45.0)·g2(58.5)와 비슷한 범위, 오염 아님(고착 아님). 그럼에도 보류하는 이유 3가지: (1) **g6가 지금 유일한 "다른 장소" held-out 증거다** — g1~g5는 전부 집이라, 학습에 넣으면 "학습 안 한 장소에서도 일반화되는가"를 확인할 방법이 없어진다(다음 다른 장소 세션이 생길 때까지). (2) **학습 재료로도 치우쳐 있다** — 라벨 0/1/2/3 = 245/666(67%)/81/4, 심각이 4개뿐이라 심각 학습엔 거의 기여 못 함, 폰 1대라 throughput·bitrate 등 feature가 폰 2대 세션과 체계적으로 다름(8/28 고착 런 같은 "세션 정체성이 feature에 새는" 위험과 같은 종류, 정도는 약함). (3) **내일 g7(같은 학교, 폰 2대)이 더 나은 재료** — occupancy 주도 심각 표본이 더 나올 가능성이 높고, g6+g7을 함께 넣으면 "학교=폰 1대"라는 잘못된 상관 대신 "이 장소에서 폰 대수가 달라도 같은 패턴"을 모델이 배울 기회가 생김.

**제안 순서**: (1) g7 수집 → (2) g7도 학습에 안 넣은 채로 기존 7세션 모델로 zero-shot 평가(g6와 동일한 절차) → (3) **g6+g7 포함 모델 vs 미포함 모델을 만들어 g7로 비교**(지금은 비교할 두 번째 학교 세션이 없어 이 실험 자체가 불가능 — g7이 있어야 "학교 데이터를 학습에 넣는 게 실제 도움되는가"를 직접 확인 가능) → (4) 도움이 확인되면 배포 모델에 g6+g7 반영.
