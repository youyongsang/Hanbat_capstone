...
**남은 것**: (세션 1c·2·2b 수집 및 풀 구성 완료) 기존 5GHz 모델을 독립 test 풀에 적용한 평가(다음 항목에서 진행) / 집 2.4GHz 세션 / 학교 방문(다른 시간대·AP 휴식 후, heavy75 2회+소패킷 반복으로 심각 독립 구간 확보) / 집 2.4GHz 세션 / 학교 방문 / Pi 재부팅 원인. 데이터·CSV·러너는 커밋하지 않음(컨벤션).
