...
**남은 것**: (세션 1c·2·2b 수집·풀 구성·독립 test 평가 완료) 집 2.4GHz 세션 수집 후 같은 방식 평가(밴드 간 공정 비교) / 학교 방문 / 낮·밤 세션 조건 차이 원인 / Pi 지연 재측정(5GHz)(다른 시간대·AP 휴식 후, heavy75 2회+소패킷 반복으로 심각 독립 구간 확보) / 집 2.4GHz 세션 / 학교 방문 / Pi 재부팅 원인. 데이터·CSV·러너는 커밋하지 않음(컨벤션).
