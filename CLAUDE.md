...
정확한 최신 수치는 `.work-log/current.md`를 확인한다(세션마다 갱신됨). 2026-08-24 아침 세션 기준 `metrics_v2.csv` 1711행, 최근 평가는 전체 정확도 67.0%대, Label 3 recall이 세션마다 20~67%로 흔들리는 중(표본이 5~6개 수준이라 통계적으로 안정적이지 않음).

### 알려진 한계

- Label 3(심각) 샘플이 여전히 얇다 — recall이 실행마다 크게 흔들린다.
- Label 2/3 트레이드오프가 절벽형이라 class weight power 중간값 튜닝이 잘 안 먹힌다.
- AP(Opal) 장비가 특정 조건에서 반복적으로 크래시한다. "몇 대가 붙었는가"보다 "누가 송신하는가"가 핵심 변수라는 가설이 있었으나(노트북 송신은 안정적, 특정 폰(191) 송신은 거의 즉시 크래시), 다른 폰(S26)으로 교체했을 때는 재현되지 않아 "191 폰 개별 하드웨어 문제였을 가능성" 쪽으로 무게가 실리는 중. 상세·최신 상황은 `docs/yongsang/ap_crash_analysis.md`와 `.work-log/current.md` 참고.
