...
```text
project/scripts/collect_metrics.py                     라이브 수집 (victim 프로브 ProbeRunner + 지속 SSH APPoller + congestion_score 계산). 2026-09-13(24차): `sta_expected_thr_mbps_mean`/`sta_tx_mcs_mean` 2컬럼 추가(정보용, 모델 입력 아님 — 9-feature 실험 5시드 전부 손해로 기각, 상세 `.work-log/current.md` 24차)
...
**핵심 논리 — LSTM이 왜 필요한가**: `max(표준 앵커)`는 그 자체로 규칙 기반 분류기다. 그런데 배포 시스템엔 victim 프로브가 없다(협조 싱크 + 지속 스트림 필요). 모델은 **AP-side 텔레메트리(점유율·retry·RSSI·bitrate)만 보고 "지금 victim QoS가 깨지고 있는가"를 예측**해야 한다 — 이건 규칙으로 못 한다. 거기에 시계열 추세 + Early Exit 효율. **반론 검토(2026-09-13)**: "공장 장비가 자기 지연율을 이미 안다면?" — 유선 산업 프로토콜(PROFINET/OPC UA)은 실제로 지연 진단을 갖지만 대부분 유선 필드기기용이라 AP가 관리하는 무선 클라이언트엔 해당 안 됨 + 채널 전환 판단은 한 클라이언트가 아니라 채널 공유 전체의 문제라 전원 협조는 비현실적 — 그래서 이 프로젝트의 "AP만 보고 판단" 전제는 유효. 선행연구(APDT 등)도 같은 이유로 AP-side 텔레메트리만 씀. 상세·인용: `docs/yongsang/congestion_label_redesign.{md,html}` §5.
...
- **세션 단위 독립 test (28차 후속, 2026-09-20 새 세션 `g1`, 972창·심각 독립 구간 6개)**: 위 수치는 같은 수집 내 분할이라 낙관적 — 기존 배포 모델을 새 세션에 적용하면 **정확도 75~80%, L3 recall 55~72%**(Baseline 60.5 / SDN 55.0 / EE Fixed 71.6 / EE Dynamic 64.3, 95% CI 약 [33~57, 74~84]로 매우 넓음). 오탐의 96%가 혼잡→심각, 심각 유형별로는 occupancy 주도 73.6% vs occ 미달 loss 주도 21~34%. 2.4GHz 캐노니컬은 실제로 2일치(8/28·9/1)·4시나리오 계열뿐이고 step 런 4~6(34%)은 `sta_tx_bitrate_mean`이 6.5Mbps로 고착된 채 수집됨(7-feature vs 6-feature 어블레이션: L3 F1 86.7 vs 80.4). 발표·보고서에서 2.4GHz L3 수치를 인용할 땐 세션 내 분할 수치와 이 독립 세션 수치를 함께 제시할 것. 상세: `.work-log/current.md` 28차 후속.
...
- **k2m2 게이트 후 남은 벽**: 재확인 필요(위 "남은 오답의 정체" 참고). k3m2 시절엔 지속형 2↔3(occ 57~69) + occ 72~73 경계로 분석됐고, k2m2도 비슷한 구간일 것으로 추정되나 오답 분해를 다시 안 함. **단일 배포 정확도는 k2m2로 오히려 낮아졌다(운 나쁜 draw)** — 4-class 95% 완주 카드는 22차(같은 종류 데이터 추가, 이득≈0)·24차·25차(feature 3종 — expected throughput/MCS/poll_interval_s — 전부 5시드 검증에서 손해~노이즈로 기각, `.work-log/current.md` 24~25차)로 전부 소진됨. **2026-09-13(24차)부터 목표1을 L3 recall/precision 메인 지표로 재정의**해 이 벽을 우회 — 위 "정량적 목표" 참고. **2026-09-14(25차 후속)** — 라벨별 occupancy-원인 비율 실측(label0/1/2는 90%+, **label3만 21.8%**로 문제가 심각 라벨에만 압도적으로 쏠림 확인) 후, **"패시브 LSTM이 확신 낮을 때만 짧게 능동 프로브 발동"하는 Early-Exit식 하이브리드 구조**를 캐노니컬 test 365창(진짜 probe 정답 보유)으로 오프라인 시뮬레이션 — **트리거율 3.4%로 4-class 정확도 93.2%→95.0%(목표1 재달성)**, 8.3%면 96.4%. 5시드 검증 완료. 실제 구현(협조기기 배치)은 스코프 밖, 향후 연구로 제시. 상세: `docs/yongsang/congestion_label_redesign.{md,html}` §5 "남는 관측 한계를 넘는 법", `.work-log/current.md` 25차 후속. 그 외 남은 대안(더 나은 AP의 per-station airtime — 일부만 하드웨어 세대 문제로 확인, 일부는 보편적 한계, forecast 재프레이밍, 시드 재추첨)은 낮은 우선순위로 보류.
...
- **데이터**: 집에서 5,810행 수집(정상 2,535/경고 2,008/혼잡 761/심각 506) → `project/data/ap_metrics_v2_5g_home/`(train 3,849/val 824/test 825).
- **3모델 5시드 결과** (test 825창): Baseline acc 88.7±0.3·L3 recall 71.1±7.9·F1 70.8±3.3 / SDN 88.1±1.0·73.4±4.7·**71.7±2.8** / EE Fixed 87.5±0.5·**73.7±4.5**·69.3±1.2 / EE Dynamic 87.0±0.5·71.8±4.8·67.8±2.2. **2.4GHz 캐노니컬보다 정확도·L3 recall 둘 다 낮음** — 당초 가설(장면 다양성 부족, 하룻밤 한 세션뿐)은 27차에서 약해졌다 — 2.4GHz도 같은 집 데이터이고 6,993행으로 늘려도 L3 recall이 오르지 않았다. 현재 유력한 설명은 아래 27차 항목(occupancy 미달 loss 주도 심각 비율·보조 feature 기여 감소).
- **남은 것**: 학교 수집(계획: 집/학교 각 2.4G+5G 2500행씩, 총 1만) 착수 전, Pi 지연 재측정(5GHz 라디오) 미실시, "혼잡 시 5GHz로 실제 전환"이라는 메커니즘 자체는 전혀 미검증(오늘은 폰을 수동으로 5GHz에 붙여 데이터만 모았을 뿐).
- **6,993행 재학습 (27차)**: 스텝형·소패킷 시나리오를 더해 재학습(test 995창)했으나 정확도 88~91%(소폭 상승), **L3 recall은 65~70%로 오히려 하락** — 데이터 추가로 개선 안 됨. 원인 분석: label 3 중 "occupancy 미달(occ_score<0.75)인데 loss 등 다른 축으로 붙은 심각" 비율이 5GHz 68% vs 2.4GHz 39%. occupancy-only 어블레이션에서 보조 feature가 L3 recall에 더해주는 이득이 5GHz +7pt vs 2.4GHz +16pt — 5GHz는 AP 텔레메트리만으론 심각 예측이 더 어렵다는 가설과 일치(메커니즘 확정은 아님). test L3의 72%가 `heavy75` 시나리오라 이를 제외하고 재학습·평가했으나 L3 recall이 오히려 49~57%로 낮아져 시나리오 쏠림은 원인이 아님(단 test L3 22개라 표본 작음).
- **세션 단위 독립 test (28차)**: 새 세션(1c·2·2b, 2,363창, 심각 독립 구간 17개)에 기존 5GHz 모델을 적용하니 **L3 recall 65~70% → 42~48%**(95% CI 약 [18~21, 58~68]로 매우 넓음), 정확도 88~91% → 85~87%. 같은 세션 내 분할 수치가 낙관적이었음 — 5GHz 결과는 이 독립 test 수치를 함께 제시할 것. 소패킷 60M은 L3 recall 75~90%, heavy60/70/75는 2~28%로 "occupancy 미달 loss 주도 심각은 AP 텔레메트리로 못 잡는다"는 가설과 방향 일치. 같은 프로파일도 밤/낮 세션에 따라 심각 발생이 갈림(원인 미확인). 2.4GHz는 세션 단위 test가 아직 없어 공정 비교 불가.
- 상세: `.work-log/current.md` 26차·26차 후속·27차·28차.
