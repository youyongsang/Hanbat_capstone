...
정확한 최신 수치는 `.work-log/current.md`를 확인한다(세션마다 갱신됨). 2026-08-24 오후 세션 기준 `metrics_v2.csv` 2111행(data rows 2110), 최근 평가는 전체 정확도 80.5%, Label 3 recall 28.6%(test 7개 중 2개) — 표본이 7개로 늘긴 했지만 여전히 세션마다 크게 흔들릴 수 있는 수준.

### 알려진 한계

- Label 3(심각) 샘플이 여전히 얇다 — recall이 실행마다 크게 흔들린다.
- Label 2/3 트레이드오프가 절벽형이라 class weight power 중간값 튜닝이 잘 안 먹힌다.
- AP(Opal) 장비가 특정 조건에서 반복적으로 크래시한다. 2026-08-24 오후 세션 기준으로는 **"다중 station(2대 이상 동시 부하)"이 다시 핵심 변수 쪽으로 무게가 실림** — S26/191 각각 단독으로는 40~150Mbps까지 전부 크래시 없이 완주했지만(191이 오히려 label 3을 더 잘 만듦), 191+S26 동시 부하 조합에서만 크래시(SSH 완전 타임아웃)가 발생했다(2회 중 1회). "191 개별 하드웨어 문제"였다는 이전 가설은 191의 5단계 연속 생존으로 사실상 반증됨. 상세·최신 상황은 `docs/yongsang/ap_crash_analysis.md`와 `.work-log/current.md` 참고.
