...
- **2026-08-24 팀 결정: 1차 `ap_cleaned_strict`는 재라벨링하지 않는다.** 1차는 1학기 모델링 파이프라인 검증용이었고 실측이 아닌 인터넷 공개 데이터 기반이었다는 한계가 이미 있어, 새 가중치로 다시 맞추기보다 archived 상태로 고정한다. 앞으로의 실측 기반 라벨링/재조정은 전부 2차(`ap_metrics_v2`)에서만 진행한다.
