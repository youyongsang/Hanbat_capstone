"""Export SDN LSTM checkpoints to ONNX deployment files.
...
DEFAULT_EXPORTS = [
    (
        PROJECT_ROOT / "checkpoints" / "sdn_lstm_best.pth",
        PROJECT_ROOT / "checkpoints" / "sdn_fixed.onnx",
        "sdn_fixed",
    ),
]
...
class SDNOnnxWrapper(nn.Module):
    ...
    def __init__(self, model: SDNLSTM) -> None:
        super().__init__()
        self.model = model
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export SDN LSTM checkpoint to ONNX.")
    parser.add_argument(
        "--checkpoint",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=PROJECT_ROOT / "checkpoints" / "sdn_fixed.onnx",
    )
    parser.add_argument(
        "--staged",
        action="store_true",
        default=True,  # 실측 및 변수 통제를 위해 기본동작으로 Stage 단위 파일 분할 활성화
        help="Also export stage1/stage2/stage3 ONNX files for real Early Exit deployment.",
    )
    return parser.parse_args()


def load_model(checkpoint_path: Path) -> SDNLSTM:
