"""Evaluate AP measurement Early Exit LSTM."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate AP Early Exit LSTM.")
    parser.add_argument("--data-dir", type=Path, default=PROJECT_ROOT / "data" / "ap_metrics")
    parser.add_argument(
        "--checkpoint",
        type=Path,
        default=PROJECT_ROOT / "checkpoints" / "ap_early_exit_lstm_best.pth",
    )
    parser.add_argument("--batch-size", type=int, default=32)
    parser.add_argument(
        "--output",
        type=Path,
        default=PROJECT_ROOT / "results" / "yongsang" / "ap_early_exit_eval_report.txt",
    )
...
def main() -> None:
    ...
    lines = [
        "AP Early Exit LSTM Evaluation Report",
        f"Data Directory: {display_path(args.data_dir)}",
        f"Checkpoint: {display_path(args.checkpoint)}",
    ]
    append_report(lines, "=== AP Early Exit + fixed theta ===", fixed_metrics)
    append_report(lines, "=== AP Early Exit + dynamic theta ===", dynamic_metrics)
