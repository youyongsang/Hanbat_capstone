"""Export AP measurement (9-feature) checkpoints to ONNX deployment files.

Covers the Baseline LSTM (single graph, no early exit) and the Proposed
Early Exit LSTM (Fixed/Dynamic checkpoints). The full model exports all
three exit heads in one graph; stage exports split the model into
sequential ONNX graphs so Raspberry Pi inference can skip deeper LSTM
layers after an early stop.
"""
...
def export_baseline(checkpoint_path: Path, onnx_path: Path) -> None:
    if not checkpoint_path.exists():
        raise FileNotFoundError(f"Checkpoint not found: {display_path(checkpoint_path)}")

    checkpoint = torch.load(checkpoint_path, map_location="cpu")
    model = BaselineLSTM(input_size=INPUT_SIZE, hidden_size=128, num_classes=4)
    model.load_state_dict(checkpoint["model_state_dict"])
    model.eval()
    ...
    print(f"ONNX export complete (ap_baseline): {display_path(onnx_path)}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export AP Baseline/Early Exit LSTM checkpoints to ONNX.")
    parser.add_argument(
        "--skip-baseline", action="store_true", help="Skip exporting the Baseline LSTM ONNX graph."
    )
    parser.add_argument(
...
def main() -> None:
    ...
    if not args.skip_baseline:
        export_baseline(
            CHECKPOINT_DIR / "ap_baseline_lstm_best.pth",
            CHECKPOINT_DIR / "ap_baseline.onnx",
        )

    for checkpoint_path, onnx_path, name in DEFAULT_EE_EXPORTS:
