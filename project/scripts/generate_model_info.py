...
현재 값 = Stage 2 결과 기반 (result_text_analysis.md 기준)
"""
...
# ── Stage 2 결과 기반 현재 파라미터 ──────────────────────────────────────
# (result_text_analysis.md 및 yongsang/stage2_work_log.md 참고)
...
model_info = {
    ...
    "dynamic_high_variance": 0.15,    # 정규화 스케일 0~1 기준
    "dynamic_mid_variance":  0.07,
    "dynamic_min_threshold": 0.1,
    "dynamic_recent_steps":  5,
    "dynamic_spike_threshold": 0.2,

    # ── 학습 결과 ─────────────────────────────────────────────────────────
    "best_val_accuracy": 0.947,       # Stage 2 학습 최고 val accuracy

    # ── 최종 테스트 성능 (Stage 2 기준, Stage 3 튜닝 후 업데이트 필요) ──
    "test_accuracy_fixed":   0.960,   # ← Stage 3 튜닝 후 수정
    "test_accuracy_dynamic": 0.967,   # ← Stage 3 튜닝 후 수정

    # ── Stage 3 튜닝 메모 ─────────────────────────────────────────────────
    "_note": (
        "Stage 2 결과 기준 초기값. "
        "Stage 3 튜닝 완료 후 test_accuracy_fixed, test_accuracy_dynamic 및 "
        "dynamic_* 파라미터 최종값으로 업데이트할 것."
    ),
