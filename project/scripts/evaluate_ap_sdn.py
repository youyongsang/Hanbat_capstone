"""Evaluate AP measurement SDN-style Early Exit LSTM inference mode."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate AP SDN-style LSTM.")
    parser.add_argument("--data-dir", type=Path, default=PROJECT_ROOT / "data" / "ap_metrics_cleaned_strict")
    parser.add_argument(
        "--checkpoint",
        type=Path,
        default=PROJECT_ROOT / "checkpoints" / "ap_cleaned_strict" / "ap_sdn_lstm_best.pth",
    )
    parser.add_argument("--batch-size", type=int, default=32)
    parser.add_argument(
        "--output",
        type=Path,
        default=PROJECT_ROOT / "results" / "yongsang" / "ap_sdn_cleaned_strict_eval_report.txt",
    )
