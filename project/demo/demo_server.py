...
iperf3 -s 서버(5201/5202)는 서버 시작 시 자동 기동.
"""
...
from scripts.collect_metrics import (  # noqa: E402
    ...
    calculate_station_deltas, parse_ap_cycle,
    summarize_stations,
)

# ---------------------------------------------------------------- config
HOST, PORT = "0.0.0.0", 8000
IPERF_TARGET = "192.168.8.226"          # 노트북 (폰 iperf3 대상)
PHONE_PORTS = {"s21": 5201, "s26": 5202}
MODEL = PROJECT_ROOT / "checkpoints" / "ap_v2_redesign2" / "ap_early_exit_fixed_unified_int8_v2.onnx"
SCALER = PROJECT_ROOT / "data" / "ap_metrics_v2_redesign2" / "scaler_params.json"
FEATURES = (
...
def inference_loop() -> None:
    global _state
    scaler = json.loads(SCALER.read_text(encoding="utf-8"))
    ...
    streak: deque = deque(maxlen=CONFIRM)
    ...
    while True:
        ...
        streak.append(raw)
        if len(streak) == CONFIRM and len(set(streak)) == 1 and streak[0] != confirmed:
            confirmed = streak[0]
...
def set_load(rate: str) -> dict:
    ...
    results = {}
    for host, port in PHONE_PORTS.items():
        _ssh(host, "pkill -f iperf3 2>/dev/null; true")
        if rate != "off":
            cmd = (f"nohup iperf3 -u -c {IPERF_TARGET} -p {port} -b {rate} -l 1400 "
                   f"-t 3600 >/dev/null 2>&1 & echo started")
            rc, out = _ssh(host, cmd)
            results[host] = "started" if rc == 0 else f"실패: {out}"
        else:
            results[host] = "stopped"
    _current_load = rate
    with _lock:
        if _state.get("ready"):
            _state["load"] = rate
...
def main() -> None:
    if not MODEL.exists() or not SCALER.exists():
        sys.exit(f"모델/스케일러 없음: {MODEL}  {SCALER}")

    iperf_procs = []
    for port in PHONE_PORTS.values():
        iperf_procs.append(subprocess.Popen(
            ["iperf3", "-s", "-p", str(port)],
            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
        ))
    print(f"iperf3 서버 기동: {list(PHONE_PORTS.values())}")

    threading.Thread(target=inference_loop, daemon=True).start()

    srv = ThreadingHTTPServer((HOST, PORT), Handler)
    print(f"데모 서버: http://localhost:{PORT}/   (Ctrl-C 종료)")
    try:
        srv.serve_forever()
