# AP 실측 2차 파이프라인 — 이 문서는 stale

> **이 문서(9-feature · weighted-sum congestion_score 시절)는 더 이상 유효하지 않다.**
> 라벨 정의와 feature가 그 뒤 두 번 바뀌었다 (9→6→7 feature, 가중합 → `max(표준 앵커)` + victim 프로브).
...
원래 내용은 git 히스토리에 있다 (`git log -p project/README_AP_V2.md`).
