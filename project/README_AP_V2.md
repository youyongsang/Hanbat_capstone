...
- **SDN-style / Baseline 비교 없음** — 이 문서가 다루는 `ap_metrics_v2`(9-feature, 구) 라인에는 여전히 없다. 이후 재설계 라인(`ap_metrics_v2_redesign2`, 6-feature)에는 **2026-08-28에 추가 완료**됐다 — `train_ap_baseline_lstm.py`/`train_ap_sdn.py`(class-weight-power=1.0으로 공정 비교), Pi INT8 실측까지 포함. 상세: `project/results/yongsang/ap_model_comparison_redesign2.txt`, `.work-log/current.md`.
