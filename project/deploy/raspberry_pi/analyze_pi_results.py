...
def latency_summary(df: pd.DataFrame) -> dict[str, float]:
    ...
    if "inference_std_ms" in df.columns:
        summary["avg_sample_std_ms"] = round(float(df["inference_std_ms"].mean()), 6)
    if "measurement_repeats" in df.columns:
        summary["measurement_repeats"] = int(df["measurement_repeats"].iloc[0])
    return summary
...
def write_markdown(
    ...
    lines = [
        ...
        f"- measurement repeats: {overall.get('measurement_repeats', 1)}",
        ...
        f"- avg sample std: {overall.get('avg_sample_std_ms', 0.0):.6f} ms",
...
def write_text(path: Path, overall: dict[str, float], exit_df: pd.DataFrame) -> None:
    lines = [
        "Raspberry Pi inference analysis",
        f"samples: {overall['samples']}",
        f"measurement_repeats: {overall.get('measurement_repeats', 1)}",
        f"accuracy: {percent(overall['accuracy'])}",
        f"avg_inference_ms: {overall['avg_inference_ms']:.6f}",
        f"avg_sample_std_ms: {overall.get('avg_sample_std_ms', 0.0):.6f}",
