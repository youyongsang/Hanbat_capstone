...
```python
def save_results(model_name, accuracy, inference_time, exit_rates, save_path):
    """
    Args:
        model_name: 모델 이름 (예: 'baseline_lstm')
        accuracy: 전체 정확도
        inference_time: 평균 추론 시간 (ms)
        exit_rates: Exit별 종료율 딕셔너리 (Early Exit만 해당)
        save_path: 저장 경로
    """
```
...
- [ ] `test_with_scenario.csv` 생성 완료 (시나리오 컬럼 추가)
- [ ] `utils/logger.py` 구현 완료
