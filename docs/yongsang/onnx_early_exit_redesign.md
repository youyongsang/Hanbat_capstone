...
>
> **(업데이트 — 2026-08-30, class-weight-power=0.0 승격 + SDN 논문 충실 재구현)** **이 문서의 방법론적 결론(staged가 세션 호출 오버헤드로 느림 → unified If 노드 → INT8은 stage별 양자화 후 재조립)은 그대로 유효.** 그 뒤 바뀐 것: ①`--class-weight-power` 기본값 1.0→0.0 (재스윕에서 정확도·Label3 F1 둘 다 최고), 세 모델 재학습·5시드 특성화, Baseline 배포 seed0→seed3 교체. ②SDN 비교모델을 Kaya et al.(ICML 2019) 논문대로 재구현(pooling IC + 램프 depth-weighted loss + val 캘리브레이션 T, base 백본만 공유). ③**Pi INT8 재측정 완료** (test 310창): Baseline 0.746ms / SDN(논문) 0.572ms / Proposed Fixed **0.540ms** / Dynamic 0.555ms — 전부 <1ms, EE가 Baseline보다 −28%. 5시드 평균 정확도는 셋이 겹침(92.0 / 90.4 / 90.7%). unified fp32는 PyTorch와 310/310 일치, INT8 v2는 306~309/310. 최신 경위: `.work-log/current.md` 7~9차, 아티팩트 "Class-Weight-Power Zero" · "AP 혼잡 분류 모델 비교".
