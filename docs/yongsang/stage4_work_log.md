...
이 방식은 계산 자체는 가볍지만, 고정 θ와 비교하면 여전히 조건 분기와 threshold 재계산이 추가된다. 최신 실측 결과에서 동적 θ의 평균 추론 시간이 고정 θ보다 0.0012ms 느린 이유는 이 추가 연산과 Python 레벨 분기 비용 때문으로 해석할 수 있다.
...
현재 delta 기반 계산은 가볍지만, 고정 θ와 비교하면 여전히 동적 계산 비용이 있다. 향후에는 다음 방식으로 더 줄일 수 있다.
